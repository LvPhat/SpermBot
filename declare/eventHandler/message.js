...
function default_1({ api, loadedCmds, loadedEvents }) {
    ...
    return async function (event, botData) {
        ...
        async function refresh(id){
          ...
          let { participantIDs: allMembers, threadName: nameThread } = await api.getThreadInfo(threadID);
        ...
        if (loadedCmds.some(item => item.name == event.command)) {
            ...
            try {
                if (cmd.adminRequired)
                    return allAdmins.includes(event.senderID) ? cmd.default({ event, botData, api, getThread, getUser, botID, allAdmins, threadAdmins, loadedCmds, loadedEvents, audioData, getUserByLink, videoData, refresh }) : api.sendMessage('Quyền lồn biên giới.', event.threadID, event.messageID);
                if (cmd.threadAdminRequired)
                    return threadAdmins.includes(event.senderID) ? cmd.default({ event, botData, api, getThread, getUser, botID, allAdmins, threadAdmins, loadedCmds, loadedEvents, audioData, getUserByLink, videoData, refresh}) : api.sendMessage('Quyền lồn biên giới.', event.threadID, event.messageID);
                return cmd.default({ event, botData, api, getThread, getUser, botID, allAdmins, threadAdmins, loadedCmds, loadedEvents, audioData, getUserByLink, videoData, refresh});
            }
            catch (err) {
