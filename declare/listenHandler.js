...
var loadedCmds = [];
const cmdFiles = fs_extra_1.readdirSync(`${__dirname}/../cmds`).filter((item) => item.endsWith('.js') && !item.includes('test'));
for (const file of cmdFiles) {
    try {
        const cmd = require(`../cmds/${file}`);
        const cmdName = cmd.name;
        //Check disabled cmd
        if (!JSON.parse(fs_extra_1.readFileSync('./data.json', { encoding: 'utf-8' })).disabled.includes(cmdName))
            if (!loadedCmds.some((item) => item.name == cmdName))
                loadedCmds.push(cmd);
            else
                delete require.cache[require.resolve(loadedCmds.find(item => item.name == cmd.location).location)];
    }
    catch (err) {
        log_1.default(`Đã có lỗi xảy ra khi tải file "${file}":\n${err.toString()}`, -1);
    }
}
...
function default_1(api) {
    ...
    return async function handleEvents(error, event) {
        if (error)
            return log_1.default(`Đã có lỗi xảy ra: ${error.error}`, -1);
        //Get bot and cmd data
        var botData = JSON.parse(fs_extra_1.readFileSync('./data.json', { encoding: 'utf-8' }));
        var cmdData = JSON.parse(fs_extra_1.readFileSync('./cmdMsg.json', { encoding: 'utf-8' }));
        switch (event.type) {
            case 'message':
            case 'message_reply': {
                ...
                let { type, body, mentions, attachments, senderID, threadID, messageID, messageReply, timestamp } = event;
                ...
                if (body.indexOf(prefix) == 0) {
                    ...
                    args = body.slice(prefix.length, body.length).split(' ').slice(1);
                    contentMsg = body.slice(prefix.length + command.length + 1, body.length);
                    if (command.includes('\n')) {
                    ...
                    if (command == 'rerun' && event.type == 'message_reply') {
                        let eventData = cmdData.find(i => i.messageID == messageReply.messageID);
                        ...
                        if (!cmdData.some(item => item.timestamp == timestamp)) {
                            cmdData.push(event);
                            fs_extra_1.writeFileSync('./cmdMsg.json', JSON.stringify(cmdData, null, '\t'));
                        }
                    }
                    let getCorrect = compare_1.default(command, loadedCmds.map(item => item.name));
                    if (getCorrect != 'none')
                        command = getCorrect;
                }
                else
                ...
                if (!botData.threads.some(item => item.id == formattedEvent.threadID)) {
                    let { participantIDs: allMembers, threadName: nameT } = await api.getThreadInfo(event.threadID);
                ...
                for (let item of botData.disabled)
                    if (event.command == item)
                        return api.sendMessage('Lệnh này không khả dụng.', event.threadID, event.messageID);
                if (eventLog && formattedEvent.senderID != api.getCurrentUserID())
                    console.log(formattedEvent);
                message_1.default({ api, loadedCmds, loadedEvents })(formattedEvent, botData);
                break;
            }
