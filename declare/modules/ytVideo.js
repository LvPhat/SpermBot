...
async function default_1(event, videoData, api) {
    ...
    if (event.contentMsg.length == 1 && parseInt(event.contentMsg) >= 1 && parseInt(event.contentMsg) <= 5) {
	        var vID = videoData.find(item => item.id == event.messageReply.messageID).link[event.contentMsg - 1];
            ...
            try {
              api.sendTypingIndicator(event.threadID, () => ytdl(videoId).pipe(createWriteStream(`./${videoId}.mp4`)).on("close", () => out({ body: videoInfo.videoDetails.title, attachment: createReadStream(`./${videoId}.mp4`) }, () => unlinkSync(`./${videoId}.mp4`))).on("error", (e) => out(e)));
        ...
        return api.sendMessage('Hãy nhập một số từ 1 đến 5.', event.threadID, event.messageID);
}
