...
async function default_1({ event, api, botData }) {
    ...
    for (const i of getThread.ban.users) {
        for (const j of event.logMessageData.addedParticipants) {
            ...
            if (i.id == +id) {
                try {
                   api.sendMessage(`@${name} không thể tham gia nhóm này vì đã bị cấm${i.reason ? ` với lí do: ${i.reason}` : '.'}`, event.threadID, () => api.removeUserFromGroup(id, event.threadID));
                  }   
                catch {
                    return log_1.default(`Không thể xóa ${name} khỏi nhóm ${event.threadID} vì không phải là Quản Trị Viên.`, -1);
                  }
               }
            }
         }
    ...
    if (event.threadID == 3720912944664252) {
        ...
        var msg = `Chào mừng ${nameArray.join(', ')} đã đến với Nhóm Hỗ Trợ/Hỏi Đáp về Mirai. ${memLength.length > 1 ? 'Các bạn' : 'Bạn'} là thành viên thứ ${memLength.join(', ')} của nhóm.\n` +
            `  + /rule: Xem luật nhóm.\n` +
            `  + /download: Link tải Mirai các phiên bản.`;
        return api.sendMessage({ body: msg, mentions }, event.threadID, (_e, i) => setTimeout(() => api.unsendMessage(i.messageID), 300000));
    }
}
